...
--MD5(CONCAT('||',
    ...
    {%- else -%}
        {{- "CAST({}(NULLIF(CONCAT('{}',".format(hash_alg, concat_string) | indent(4) -}}
    {%- endif -%}
...
{%- else -%}
    ...
    {%- if is_hashdiff -%}
        {{- "CAST(HASHBYTES('{}', (CONCAT('{}',".format(hash_alg, concat_string) | indent(4) -}}
    {%- else -%}
        {{- "CAST(HASHBYTES('{}', (NULLIF(CONCAT('{}',".format(hash_alg, concat_string) | indent(4) -}}
    {%- endif -%}
...
{%- elif hash_alg == 'SHA256' -%}
    ...
    {%- set hash_expr_right = " AS VARCHAR)), 'hex')" -%}
{%- endif -%}
...
{%- if columns is string -%}
    ...
    {{- "CAST(UPPER({}{}{}) AS VARCHAR) AS {}".format(hash_expr_left, standardise | replace('[EXPRESSION]', escaped_column_str), hash_expr_right, dbtvault.escape_column_names(alias)) | indent(4) -}}
...
{%- else -%}
    ...
    {%- if is_hashdiff -%}
        {{- "CAST(UPPER({}CONCAT('{}',".format(hash_expr_left, concat_string) | indent(4) -}}
    {%- else -%}
        {{- "CAST(UPPER({}NULLIF(CONCAT('{}',".format(hash_expr_left, concat_string) | indent(4) -}}
    {%- endif -%}
    ...
    {%- for column in columns -%}
        ...
        {%- if loop.last -%}
            ...
            {% if is_hashdiff %}
                {{- "\n){}) AS VARCHAR) AS {}".format(hash_expr_right, dbtvault.escape_column_names(alias)) -}}
            {%- else -%}
                {{- "\n), '{}'){}) AS VARCHAR) AS {}".format(all_null | join(""), hash_expr_right, dbtvault.escape_column_names(alias)) -}}
            {%- endif -%}
        {%- else -%}
...
{%- else -%}
    ...
    {%- if is_hashdiff -%}
        {%- if hash_alg == "MD5" %}
            {{- "UPPER(MD5(CONCAT('{}',".format(concat_string) | indent(4) -}}
        {%- else %}
            {{- "UPPER(SHA2(CONCAT('{}',".format(concat_string) | indent(4) -}}
        {%- endif %}

    {%- else -%}
        {%- if hash_alg == "MD5" %}
            {{- "UPPER(MD5(NULLIF(CONCAT('{}',".format(concat_string) | indent(4) -}}
        {%- else %}
            {{- "UPPER(SHA2(NULLIF(CONCAT('{}',".format(concat_string) | indent(4) -}}
        {%- endif %}
    {%- endif -%}
