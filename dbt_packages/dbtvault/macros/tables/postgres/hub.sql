...
-#}
    {%- if model.config.materialized == 'vault_insert_by_rank' %}
    SELECT DISTINCT {{ dbtvault.prefix(source_cols_with_rank, 'rr') }}
    {%- else %}
    SELECT DISTINCT {{ dbtvault.prefix(source_cols, 'rr') }}
    {%- endif %}
    FROM {{ ref(src) }} AS rr
...
row_rank_union AS (
{#- PostgreSQL has DISTINCT which should be more performant than the
    strategy used by Snowflake ROW_NUMBER() OVER( PARTITION BY ...
-#}
    SELECT DISTINCT ({{ dbtvault.prefix([src_pk], 'ru') }}) ru.*
